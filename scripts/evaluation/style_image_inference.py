...
def load_data_from_json(data_dir, filename=None):
    ...
    for idx in range(n_samples):
        ...
        if data[idx]['style_path'] is not None:
            ...
            if isinstance(style_path, list) and not SINGLE_REF:
                style_imgs = []
            ...
            elif isinstance(style_path, list) and SINGLE_REF:
...
def save_results(prompt, samples, filename, img_dir, prompt_dir):
    ...
    output_img.save(os.path.join(img_dir, "%s.jpg"%filename))
...
def style_guided_synthesis(model, prompts, style, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
    ...
    if hasattr(model.adapter, "scale_predictor") and append_to_context is not None:
    ...
    for _ in range(n_samples):
        ...
        batch_images = model.decode_first_stage(samples)[:, :, 0, :, :]
        batch_variants.append(batch_images)
    ## variants, batch, c, h, w
    batch_variants = torch.stack(batch_variants)
    return batch_variants.permute(1, 0, 2, 3, 4)
...
def run_inference(args, gpu_num, gpu_no):
    ...
    data_list = load_data_from_json(args.prompt_dir, args.filename)
    ...
    for idx, indice in tqdm(enumerate(range(0, len(data_list_rank), args.bs)), desc='Sample Batch'):
        ...
        if len(styles.shape) == 4:
            for nn in range(styles.shape[0]):
                save_img(styles[nn], os.path.join(style_dir, f'{idx*args.bs+nn + gpu_no * samples_split:04d}.png'))
        else:
            for nn in range(styles.shape[0]):
                for i in range(styles.shape[1]):
                    save_img(styles[nn, i], os.path.join(style_dir, f'{idx*args.bs+nn + gpu_no * samples_split:04d}_{i:02d}.png'))
        ...
        for nn, samples in enumerate(batch_samples):
            ...
            for i in range(args.n_samples):
                save_results(prompt, samples[i:i+1], f"{filename}_{i}", sample_dir, prompt_dir)
